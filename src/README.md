...
1.  **Typescript Integration tests**. They are located inside the `tests/integration` directory and cover both Solana program functionality (`programs/`) and the Friktion SDK (`src/`). They are our best attempt at simulating real-world usage of the volts. Configuration is in `Anchor.toml`[^3]
