...
//New Struct when submitting orders
...
pub struct InitSerumMarket<'info> {
    ...
    #[account(mut)]
    pub whitelist: Box<Account<'info, Mint>>,
    ...
    #[account(init,
        seeds = [&whitelist.key().to_bytes()[..], &option_market.key().to_bytes()[..], b"serumMarket"],
        bump,
    ...
    #[account(init,
        seeds = [&whitelist.key().to_bytes()[..], &option_market.key().to_bytes()[..], b"requestQueue"],
        bump,
        space = 5120 + 12,
        payer = user_authority,
        owner = *dex_program.key
    ...
    #[account(init,
        seeds = [&whitelist.key().to_bytes()[..], &option_market.key().to_bytes()[..], b"coinVault"],
        bump,
        payer = user_authority,
    ...
    #[account(init,
        seeds = [&whitelist.key().to_bytes()[..], &option_market.key().to_bytes()[..], b"pcVault"],
        bump,
        payer = user_authority,
        token::mint = pc_mint,
        token::authority = vault_signer,
